...
type ExportDialogProps = { data: unknown; filename: string; onOpenChange: (open: boolean) => void; open: boolean; title?: string; description?: string };
...
export function ExportDialog({ data, filename, onOpenChange, open, title, description }: ExportDialogProps) {
  ...
  return <AlertDialog.Root onOpenChange={onOpenChange} open={open}><AlertDialog.Portal>
    ...
    <AlertDialog.Content className={styles.content} onOpenAutoFocus={(event) => { event.preventDefault(); copyButtonRef.current?.focus(); }}>
      <div className={styles.heading}><div><AlertDialog.Title>{title ?? t("exportTitle")}</AlertDialog.Title><AlertDialog.Description>{description ?? t("exportDescription")}</AlertDialog.Description></div><Tooltip content={t("close")}><AlertDialog.Cancel asChild><button aria-label={t("close")} className={styles.close} onPointerDown={(event) => event.preventDefault()} type="button"><X aria-hidden="true" size={18} /></button></AlertDialog.Cancel></Tooltip></div>
