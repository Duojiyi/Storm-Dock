...
import { beginAttachmentProbeScope } from "../lib/attachmentAvailability";
import { disposeViewportProbeSession } from "../lib/attachmentViewportProbe";
import { SessionAttachments } from "./SessionAttachments";
import { VirtualSessionMessages } from "./VirtualSessionMessages";
...
export const GrokBotSessionWorkspace = memo(function GrokBotSessionWorkspace({
  ...
  useEffect(() => {
    // Selected-session scope + viewport probes only; drop previous session queue.
    setProbeGeneration(beginAttachmentProbeScope(selectedId));
    disposeViewportProbeSession(messagesScrollRef.current);
    if (!selectedId) {
      setMessages([]);
      return;
    }
  ...
  if (!sessions.length) return <div className={styles.empty}><MessageSquareText aria-hidden="true" size={32} /><h2>{t("sessionsEmptyTitle")}</h2><p>{t("sessionsEmptyDescription", { application: label })}</p></div>;
